import EventRegistrationEntity from "@data/entities/event-registration.entity";
...
export class EventRegistrationRepository {
  ...
  async update(id: string, data: Partial<IEventRegistration>): Promise<IEventRegistration | null> {
    const [affectedCount] = await EventRegistrationEntity.update(data, {
      where: { id }
    });
    
    if (affectedCount === 0) return null;
    
    const updatedEventRegistration = await EventRegistrationEntity.findByPk(id);
    return updatedEventRegistration ? (updatedEventRegistration.dataValues || updatedEventRegistration) : null;
  }

  async delete(id: string): Promise<boolean> {
    const affectedCount = await EventRegistrationEntity.destroy({
      where: { id }
    });
  ...
  async findByEventAndUser(eventId: string, userId: string): Promise<IEventRegistration | null> {
    const eventRegistration = await EventRegistrationEntity.findOne({
  ...
  async countByEventId(eventId: string): Promise<number> {
    return await EventRegistrationEntity.count({
      where: { eventId }
    });
  }
}
