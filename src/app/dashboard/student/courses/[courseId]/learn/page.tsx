...
const CustomVideoPlayer: React.FC<CustomVideoPlayerProps> = ({ videoUrl, title, onProgress }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentTime, setCurrentTime] = useState(0);
  const [duration, setDuration] = useState(0);
...
const LearningPage: React.FC<LearningPageProps> = () => {
      ...
      <Layout style={{ minHeight: "100vh", backgroundColor: "#f5f5f5" }}>
        <div style={{ 
          ...
          <div style={{ color: "white", fontSize: "18px", textAlign: "center" }}>
            <div style={{ marginBottom: "8px" }}>🔐 Verifying Authentication</div>
            <div style={{ fontSize: "14px", opacity: 0.8 }}>Please wait while we verify your access...</div>
          </div>
        </div>
      </Layout>
    );
    ...
    if (progressData && Array.isArray(progressData)) {
      try {
        const completedLessonIds = [...progressData]
          .filter((progress: ILessonProgress) => 
            progress && 
    ...
    try {
      ...
      if (Array.isArray(enrollments)) {
        ...
        if (!isEnrolled) {
          ...
          setTimeout(() => {
            router.push(`/dashboard/student?message=enrollment-required&courseId=${courseId}`);
          }, 2000);
        }
      }
    } catch (error) {
  ...
  React.useEffect(() => {
    if (modulesError) {
      ...
      const isSessionError = (modulesError as any)?.data?.message?.includes?.("Session error");
      
      if (isAuthError || isSessionError) {
        console.warn("Authentication error detected, attempting to refresh session...");
  ...
  const handleQuizSubmit = async () => {
    ...
    try {
      ...
      const attemptNumber = previousSubmission ? (previousSubmission.attemptNumber || 1) + 1 : 1;
      ...
      await updateLessonProgress({
        ...
        lastAccessedAt: new Date().toISOString(),
        notes: `Quiz "${currentQuiz.title}" completed. Score: ${score}/${maxScore} (${percentage.toFixed(1)}%). Answer: "${userAnswer}" (${isCorrect ? 'Correct' : 'Incorrect'})`,
      }).unwrap();
      ...
      if (isCorrect) {
        api.success({
          ...
          description: isRetry 
            ? `Great improvement! Attempt ${attemptNumber}. ${currentQuiz.explanation || "You got the right answer!"}`
                ...
                <div style={{ 
                  ...
                  {lessonLoading ? (
                    <div style={{
                      ...
                      <Text style={{ color: "white", fontSize: "16px" }}>Loading lesson content...</Text>
                    </div>
                    ...
                    <Card style={{ 
                      ...
                      <Title level={4} style={{ color: "#667eea" }}>No video available</Title>
                      <Text style={{ color: "#8892b0" }}>This lesson is text-based. Switch to the Content tab.</Text>
                    </Card>
                  )}
                </div>
              ),
            ...
            },
            {
              ...
              children: (
                <Card style={{ minHeight: "400px" }}>
                  <Title level={4} style={{ marginBottom: "24px" }}>Course Resources</Title>
                  ...
                  {currentLesson.resources && currentLesson.resources.length > 0 ? (
                    <List
                      ...
                      renderItem={(resource: any) => (
                        ...
                        >
                          <List.Item.Meta
                            avatar={<Avatar icon={resource.type === 'pdf' ? <FileTextOutlined /> : <LinkOutlined />} />}
              ...
              children: (
                <Card style={{ minHeight: "400px" }}>
                  <div style={{ marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <Title level={4} style={{ margin: 0 }}>Course Reviews</Title>
                      {courseReviews?.stats && (
                        <div style={{ marginTop: "8px" }}>
                          <Rate disabled value={courseReviews.stats.averageRating} allowHalf style={{ fontSize: "16px" }} />
                          <Text style={{ marginLeft: "8px", color: "#666" }}>
                            {courseReviews.stats.averageRating.toFixed(1)} ({courseReviews.stats.totalReviews} reviews)
                          </Text>
                        </div>
                      )}
                    </div>
                    ...
                    <Card 
                      ...
                      title={
                        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                          ...
                          <Tag color="blue">
                            {courseReviews.userReview.status.charAt(0).toUpperCase() + courseReviews.userReview.status.slice(1)}
                          </Tag>
                        </div>
                      }
                    >
                  ...
                  {courseReviews?.reviews && courseReviews.reviews.length > 0 ? (
                    <List
                      dataSource={courseReviews.reviews.filter(review => review.id !== courseReviews?.userReview?.id)}
                            ...
                            title={
                              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                ...
                                {review.wouldRecommend && (
                                  <Tag color="green" style={{ fontSize: "11px" }}>Recommends</Tag>
                                )}
                              </div>
                            }
                            description={
  ...
  const getQuizSubmissionStatus = (quizId: string) => {
    if (!userQuizSubmissions || !Array.isArray(userQuizSubmissions)) return null;
    
    const submissions = userQuizSubmissions.filter((submission: any) => 
  ...
  const getAssignmentSubmissionStatus = (assignmentId: string) => {
    if (!userAssignmentSubmissions || !Array.isArray(userAssignmentSubmissions)) return null;
    
    const submissions = userAssignmentSubmissions.filter((submission: any) => 
  ...
  const getSubmissionStatusTag = (submission: any, type: 'quiz' | 'assignment') => {
  ...
  const renderAssignmentsQuizzes = () => {
    ...
    if (!assignmentsLoading && !quizzesLoading && !hasAssignments && !hasQuizzes) {
      return (
        <Card 
          style={{ 
            height: "100%",
          ...
          <Empty 
            description={
              <div style={{ textAlign: "center" }}>
                <Text style={{ fontSize: "16px", color: "#999", display: "block", marginBottom: "8px" }}>
        ...
        <div style={{ maxHeight: "calc(100vh - 250px)", overflowY: "auto", paddingRight: "8px" }}>
          {(assignmentsLoading || quizzesLoading) && (
            <div style={{ textAlign: "center", padding: "20px" }}>
              <Spin size="small" />
              <Text style={{ marginLeft: "8px", color: "#666" }}>Loading activities...</Text>
            </div>
          )}
          
          {!assignmentsLoading && hasAssignments && (
            <>
              <Title level={5} style={{ color: "#1890ff", marginBottom: "16px", fontSize: "16px" }}>
                📝 Assignments
              </Title>
            <List
              size="small"
              ...
              renderItem={(assignment: any) => {
                const submission = getAssignmentSubmissionStatus(assignment.id);
                      ...
                      <div style={{ marginBottom: "12px" }}>
                        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "8px" }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <Text strong style={{ fontSize: "15px", display: "block", lineHeight: "1.4" }}>
                              {assignment.title}
                            </Text>
                        ...
                        <Text style={{ fontSize: "13px", color: "#666", display: "block", marginBottom: "12px", lineHeight: "1.5" }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <Space>
                          ...
                          <Button 
                            ...
                            type="primary"
                            style={{
                              backgroundColor: hasSubmission ? "#faad14" : "#1890ff",
                              borderColor: hasSubmission ? "#faad14" : "#1890ff"
                            }}
                          >
                            {hasSubmission ? "Retry Assignment" : "Start Assignment"}
                          </Button>
                        </Space>
                      </div>
                    </div>
          ...
          {!quizzesLoading && hasQuizzes && (
            <>
              <Title level={5} style={{ color: "#52c41a", marginTop: hasAssignments ? "24px" : "0", marginBottom: "16px", fontSize: "16px" }}>
                🧠 Quizzes
              </Title>
            <List
              size="small"
                    ...
                    <div style={{ width: "100%" }}>
                      ...
                      <Space style={{ width: "100%", justifyContent: "space-between" }}>
                        <Text style={{ fontSize: "12px", color: "#666" }}>
                          1 question • {quiz.timeLimitMinutes || quiz.timeLimit || 30} min
                        </Text>
                        <Space>
                          {hasSubmission && (
                            <Button 
                          ...
                          <Button 
                            ...
                            style={{
                              borderColor: hasSubmission ? "#faad14" : "#667eea",
    ...
    return (
      <div style={{ height: "100%", overflow: "auto" }}>
          ...
          {courseReviews?.stats && courseReviews.stats.totalReviews > 0 && (
            <div style={{ marginTop: "12px" }}>
              <Rate 
                ...
                style={{ fontSize: "16px" }} 
              />
              <Text style={{ color: "rgba(255,255,255,0.9)", marginLeft: "8px" }}>
                {courseReviews.stats.averageRating.toFixed(1)} ({courseReviews.stats.totalReviews} reviews)
              </Text>
            </div>
          )}
        ...
        <div style={{ padding: "16px" }}>
          <Collapse
            defaultActiveKey={courseData.modules ? [...courseData.modules].filter(m => !m.isCompleted).map(m => m.id) : []}
            ghost
          >
                ...
                <Panel
                  ...
                  header={
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                      <div>
                        <Text strong style={{ color: module.isCompleted ? "#52c41a" : "#333" }}>
                          {module.title}
                        </Text>
                        <div style={{ fontSize: "12px", color: "#999", marginTop: "4px" }}>
                          {module.completedLessons}/{module.totalLessons} lessons • {module.estimatedDurationHours}h
                        </div>
                      </div>
                      <div>
                  ...
                  style={{
                    ...
                    border: `1px solid ${module.isCompleted ? "#b7eb8f" : "#f0f0f0"}`,
                    borderRadius: "8px"
                  }}
                >
                  <List
                    size="small"
                    dataSource={module.lessons ? [...module.lessons].sort((a, b) => (a.lessonOrder || 0) - (b.lessonOrder || 0)) : []}
                    renderItem={(lesson) => (
                      <List.Item
                        style={{
                          padding: "8px 12px",
                          ...
                          borderRadius: "6px",
                          marginBottom: "4px",
                          border: currentLesson?.id === lesson.id ? "1px solid #91d5ff" : "1px solid transparent"
                        }}
                        ...
                        <List.Item.Meta
                          ...
                          title={
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                              <Text style={{ fontSize: "14px", fontWeight: currentLesson?.id === lesson.id ? "600" : "normal" }}>
                                {lesson.title}
                              </Text>
                              <Space>
                                {completedLessons.has(lesson.id) && <CheckCircleOutlined style={{ color: "#52c41a" }} />}
                                {lesson.isFreePreview && <Tag color="green">Free</Tag>}
                              </Space>
                            </div>
                          }
                          description={
                            <Space>
                              <Text style={{ fontSize: "12px", color: "#999" }}>
                                <ClockCircleOutlined /> {lesson.durationMinutes} min
                              </Text>
                              <Text style={{ fontSize: "12px", color: "#999" }}>
                                {lesson.lessonType}
                              </Text>
    ...
    return (
      <Layout style={{ minHeight: "100vh", backgroundColor: "#f5f5f5" }}>
        <div style={{ 
          ...
          <Spin size="large" />
          <Text style={{ marginLeft: "16px", fontSize: "16px" }}>Loading course content...</Text>
        </div>
      </Layout>
    );
  }
    ...
    return (
      <Layout style={{ minHeight: "100vh", backgroundColor: "#f5f5f5" }}>
        <div style={{ 
          ...
          <Alert
            message={isSessionError ? "Session Expired" : "Unable to load course"}
            description={
              isSessionError 
                ? "Your session has expired. Please clear your browser cookies and log in again."
                : `Error loading course content: ${(modulesError as any)?.data?.message || (modulesError as any)?.message || "Unknown error"}`
            }
            type="error"
            showIcon
            action={
        ...
        <div style={{ 
          ...
          >
            ...
            <div style={{
              ...
              <Title level={2} style={{ color: "white", marginBottom: "8px", fontWeight: "700" }}>
              ...
              <Space direction="vertical" size={24} style={{ width: "100%" }}>
                ...
                <div style={{
                  ...
                  <Text style={{ 
                    ...
                    💡 <strong>Need help?</strong> Contact your instructor or check back later. 
                    Course content is regularly updated and new modules may be added soon.
                  </Text>
                </div>
              </Space>
            </div>
      ...
      <Modal
        title={
          <div style={{ 
            ...
            <QuestionCircleOutlined style={{ marginRight: "8px", fontSize: "20px" }} />
                  ...
                  <div style={{ 
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <div style={{ textAlign: "center" }}>
                        ...
                        <Text style={{ fontSize: "14px", color: "#faad14", fontWeight: "bold" }}>
                          🔄 Retaking Quiz
                        </Text>
                      </div>
                    </div>
            ...
            >
              <div style={{ marginBottom: "16px" }}>
                ...
                {currentQuiz.difficulty && (
                  <Tag 
                    color={
                      currentQuiz.difficulty === 'easy' ? 'green' : 
                      currentQuiz.difficulty === 'medium' ? 'orange' : 'red'
                    }
                ...
                {(() => {
                  try {
                    ...
                    return answers.map((option: string, optIndex: number) => (
                      <Button
                        key={optIndex}
                        type={quizAnswers[currentQuiz.id] === option ? "primary" : "default"}
                        block
                        style={{
                          textAlign: "left",
                          height: "auto",
              ...
              {currentQuiz.localExample && (
                <div style={{ 
                  ...
                  <Text style={{ fontSize: "14px", color: "#0050b3" }}>
                    <strong>🇨🇲 Local Example:</strong> {currentQuiz.localExample}
                  </Text>
                </div>
              )}
              ...
              <div style={{ 
                ...
                <span>Type: {currentQuiz.quizType?.replace('_', ' ').toUpperCase() || 'MULTIPLE CHOICE'}</span>
              </div>
            </Card>
          </div>
        )}
